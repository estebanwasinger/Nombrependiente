...
class VerDatosJugadorWindow extends Dialog<Jugador> {
	...
	def verTablas(Panel panel) {
		...
		var tablaInfracciones = new Table<Infraccion>(panel, Infraccion) => [heigth = 130 width = 280
			bindItemsToProperty("infracciones")]

		new Column<Infraccion>(tablaInfracciones).setTitle("Fecha").bindContentsToTransformer(
			[infraccion|new SimpleDateFormat("dd/MM/YYYY").format(infraccion.fecha)]).setFixedSize(75)

		new Column<Infraccion>(tablaInfracciones).setTitle("Hora").bindContentsToTransformer(
			[infraccion|new SimpleDateFormat("HH:mm:ss").format(infraccion.fecha)]).setFixedSize(75)

		new Column<Infraccion>(tablaInfracciones).setTitle("Motivo").bindContentsToProperty("motivo")
	}
